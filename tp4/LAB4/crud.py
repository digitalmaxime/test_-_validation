...
class CRUD:
    ...
    def __init__(self):
        self.users_file = dict() #"users.json"
        self.groups_file = dict() #"groups.json"        
        self.user_counter = len(self.read_users_file()) +1
        self.group_counter = len(self.read_groups_file()) +1
        self.removed_users_ids = [] # pour les id supprime. 
    ...
    def add_new_user(self, user_email, date):
        ...
        if len(self.removed_users_ids) == 0:
            user_id = self.user_counter
        else:
            user_id = self.removed_users_ids.pop()
        users_data["{}".format(user_id)] = data
    ...
    def add_new_group(self, name, trust, members_list):#pragma: no cover
        '''
    ...
    def get_user_data(self, user_id, field): #pragma: no cover
        '''
    ...
    def get_group_data(self, group_id, field):#pragma: no cover
        '''
        Description: fonction qui sorte la valeur d'information specifie
    ...
    def get_user_id(self, name): #pragma: no cover
        '''
    ...
    def get_group_id(self, name): #pragma: no cover
        '''
    ...
    def update_users(self, user_id, field, data):
        ...
        Sortie: bool, 'True' pour succes, 'False' dans le cas de failure.
        '''
        assert field in ["name", "Trust", "Spamn", "HamN", "Groups", "Date_of_first_seen_message", "Date_of_last_seen_message"], "Incorrect field argument"
        ...
        if field == "Date_of_last_seen_message":
            if len(data) != 10:                     #TODO faut-il le couvrir                       
                return False
            year = int(data[:4])
            month = int(data[5:7])
            day = int(data[8:])
            ...
            #verifier si la date est plus recente que celle de last_seen_message
            if data <= self.get_user_data(user_id, "Date_of_first_seen_message"):
                field = "Date_of_first_seen_message"
            elif data >= self.get_user_data(user_id, "Date_of_first_seen_message"):
                pass
            else:
                return False
        ...
        if field == "Groups":
            grp_id = self.get_group_id(data)
            if data not in users[user_id][field] and grp_id:
                users[user_id][field].append(data)
                group_file = self.read_groups_file()
                group_file[grp_id]["List_of_members"].append(users[user_id]["name"])
                # self.update_groups(grp_id, "List_of_members", users[user_id]["name"])
            else:
                return False

        elif field == "name":
            previous_name = users[user_id][field]
            users[user_id][field] = data
            groups = self.read_groups_file()
            for group in users[user_id]["Groups"]:
                group_id = self.get_group_id(group)
                groups[group_id]["List_of_members"].append(data)
                groups[group_id]["List_of_members"].remove(previous_name)
    ...
    def update_groups(self, group_id, field, data): #pragma: no cover
        '''
    ...
    def remove_user(self, user_id):
        ...
        self.modify_users_file(users)
        self.user_counter -= 1
        self.removed_users_ids.append(user_id)
    ...
    def remove_group(self, group_id): #pragma: no cover
        '''
    ...
    def remove_group_member(self, group_id, member): #pragma: no cover
        '''
...
# crud.add_new_user("max@", "2020-11-15")
# print(crud.user_counter)
# crud.add_new_user("maude@", "2020-11-15")
# print(crud.user_counter)
# crud.add_new_user("monique@", "2020-11-15")
# print(crud.user_counter)
# crud.remove_user("2")
# crud.add_new_user("Shanti@", "2020-11-15")
# print(crud.user_counter)

# print(crud.users_file)

# crud.update_users("1", "Date_of_last_seen_message", "2020-11-21")
# crud.update_users("1", "Date_of_last_seen_message", "2020-11-17")

# crud.update_users("3", "Groups", "Amies")