...
class VocabularyCreator:
    ...
    def __init__(self, cleaning_mode, file):
        self.train_set = file 
    ...
    def define_vocab(self):
        ...
        for email in data["dataset"]:
            ...
            subject = cleaner.clean_text(subject, self.cleaning_mode)
            body = cleaner.clean_text(body, self.cleaning_mode)
    ...
    def create_ratio(self, dict_vocab):
    ...
    def trim_vocab(self, dict_vocab, num):
        unwanted_values = [key for key in dict_vocab["spam_sub"] if dict_vocab["spam_sub"][key] < num ]
        nb_unwanted_values = len(unwanted_values)
        for key in unwanted_values:
            del dict_vocab["spam_sub"][key]
            dict_vocab["spam_sub_counter"] -= 1

        unwanted_values = [key for key in dict_vocab["spam_body"] if dict_vocab["spam_body"][key] < num ]
        nb_unwanted_values = len(unwanted_values)
        for key in unwanted_values:
            del dict_vocab["spam_body"][key]
            dict_vocab["spam_body_counter"] -= 1
        
        unwanted_values = [key for key in dict_vocab["ham_sub"] if dict_vocab["ham_sub"][key] < num ]
        nb_unwanted_values = len(unwanted_values)
        for key in unwanted_values:
            del dict_vocab["ham_sub"][key]
            dict_vocab["ham_sub_counter"] -= 1

        unwanted_values = [key for key in dict_vocab["ham_body"] if dict_vocab["ham_body"][key] < num ]
        nb_unwanted_values = len(unwanted_values)
        for key in unwanted_values:
            del dict_vocab["ham_body"][key]
            dict_vocab["ham_body_counter"] -= 1
    ...
    def create_vocab(self, num): 
        vocab = self.define_vocab()
        if num > 1:
            vocab = self.trim_vocab(vocab, num)
        vocab = self.create_ratio(vocab)
