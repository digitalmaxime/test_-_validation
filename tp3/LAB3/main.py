...
if __name__ == "__main__":
    ...
    count_line = 8
    for key in dict_csv:
        use_log = int(dict_csv[key][0])
        combinaison_log = int(dict_csv[key][1])
        vocab_counter_treshold = int(dict_csv[key][2])
        cleaning_mode = int(dict_csv[key][3])       
        calcul_mode = int(dict_csv[key][4])         # 0 ,1 ou 2
    ...
    #     #3. Evaluation de performance du modele avec la fonction evaluate()
        ...
        dict_result[count_line] = [use_log, combinaison_log, vocab_counter_treshold, cleaning_mode,calcul_mode, results[0], results[1],results[2],results[3]]
        count_line+= 1
